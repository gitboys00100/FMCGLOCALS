...
export class LoginComponent implements OnInit {
	...
	onSubmitLogin(){
		...
		if (this.loginForm.invalid) {
			...
			this.login('authenticate/', loginDetails, null);
		}
		
		
	}
	login(url: string, payload, params: string){
		this.apiService.callAPI('authenticate/', 'post', payload)
		.map((res:any) =>
		res.body
	)
	.subscribe((data) => {
		...
		localStorage.setItem('user_info', JSON.stringify(resJSON['data']['user']));
