...
export class UsersService {
    ...
    public getAgentItineraries(agent) {
        return new Promise(async (resolve, reject) => {
            this.api.get('itineraries/agents/', { agent_id: agent.id })
                .map((res: any) => res.data)
                .subscribe((data: any[]) => {
                    let itineraryAgents = [[],[],[],[],[],[],[]];
                            ...
                            for (let j = 0; j < data.length; j++) {
                                if (data[j].days[i] === "1") {
                                    for (let k = 0; k < data[j].itinerary.retailers.length; k++) {
                                        ...
                                        for (let l = 0; l < itineraryAgents[i].length; l++) {
                                            if (itineraryAgents[i][l].id === data[j].itinerary.retailers[k].id) {
                                                exists = true;
                                                break;
                                            }
                                        }
    
                                        if (!exists) itineraryAgents[i].push(data[j].itinerary.retailers[k]);
                                    }
                                }
