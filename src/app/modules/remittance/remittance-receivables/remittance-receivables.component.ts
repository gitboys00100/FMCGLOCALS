...
export class RemittanceReceivablesComponent implements OnInit {
  ...
  constructor(private data: DataCountsService, private apiService: ApiService) { }
  receivables: any[];
  agents: string;
  loading: boolean;
  ...
  openReceiptDialogModal(value: string) {
    ...
    payment.forEach(function(p){
      if(p.id == value) {
        this.selectedAgentId = p.user_id;
        this.selectedTimestamp = p.timestamp;
