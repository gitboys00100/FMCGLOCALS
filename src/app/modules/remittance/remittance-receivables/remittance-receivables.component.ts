...
export class RemittanceReceivablesComponent implements OnInit {
  ...
  alterFilter(enable: boolean) {
    var dateFilter = <HTMLInputElement>document.getElementById('date');
    var agentFilter = <HTMLInputElement>document.getElementById('agentFilter');
    if(enable) {
  ...
  openBreakdown(id: string) {
    ...
    if(specificReceivables.length <= 0) {
      this.receivables = [];
    }
    else {
