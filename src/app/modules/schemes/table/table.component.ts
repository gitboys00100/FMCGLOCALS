  ...
  this.apiService.post('discounts/',user_json_string).subscribe(ret => {
    ...
    openEditModal(e) {
      ...
      if(e.start_date == null){
        this.editForm.get('e_start').setValue('');
      }
      else{
      ...
      if(e.end_date == null){
        this.editForm.get('e_end').setValue('');
      }
      else{
...
clickFrom(date){
  let fdate = date;
  let tdate = this.addForm.get('a_end').value;

  let control = this.addForm.get('a_end');
    ...
    if(fdate == tdate){
      alert('Dates must not be equal!');
      this.addForm.get('a_start').setValue('');
    }
    else{
      if(tdate != ''){
        if(fdate > tdate){
          alert('"From date" must be earlier than "End date"!');
          this.addForm.get('a_start').setValue('');
        }
      }
    }
...
checkDates(date){
  let tdate = date;
  let fdate = this.addForm.get('a_start').value;

  //console.log(tdate + " " + fdate);
  if(fdate == tdate){
    alert('Dates must not be equal!');
    this.addForm.get('a_end').setValue('');
  }
  else{
    if(fdate <= tdate){
    }
    else{
      alert('"End date" must be later than "From date"!');
      this.addForm.get('a_end').setValue('');
    }
  }
}

public onFilterTypeChange($event) {
...
clickEFrom(date){
  let fdate = date;
  let tdate = this.editForm.get('e_end').value;

  let control = this.editForm.get('e_end');
    //control.disabled ? control.enable() : control.disable();
    if(date != ''){
      control.enable();
    }
...
checkEDates(date){
  let tdate = date;
  let fdate = this.editForm.get('e_start').value;
