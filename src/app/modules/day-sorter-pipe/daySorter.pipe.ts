      ...
      if(name == 'reports') {
        for(var x =0;x<arr.length;x++) {
          if(this.getFullDate(parseInt(arr[x].date)) == this.getFullDate(date)) {
            ...
            var TempProd = [];
            for(var y =0;y<arr[x].products.length;y++) {
              var products = {};
              products['productname'] = arr[x].products[y].productname;
              products['uom'] = arr[x].products[y].uom;
              products['requestedquantity'] = arr[x].products[y].requestedquantity;
              products['loadinquantity'] = arr[x].products[y].loadinquantity;
              products['loadoutquantity'] = arr[x].products[y].loadoutquantity;
              products['estimatedsales'] = arr[x].products[y].estimatedsales;
              TempProd.push(products);
            }
            toPush['products'] = TempProd;
            TempArr.push(toPush);
          }
