...
.subscribe(ret => {
  let data: any[] = JSON.parse('['+JSON.stringify(ret)+']');
  var arrAgents = Object.values(data);
  //console.log(arrAgents);
  for(var a of arrAgents[0]['data']){
    if(a.profile.role.name == 'Sales Agent'){
      this.agents.push({
        id: a.id,
...
openEditModal(ret) {
  ...
  this.profileForm.get('agent').setValue(ret.assigned_agents[0]['id']);
  ...
  console.log(retdata);
    ...
    .subscribe((response) => {
      if(response['code'] == 2004 && response['message'] == 'Entry updated') {
        ...
        let retailer_agent = {};
        retailer_agent['user_id'] = this.profileForm.get('agent').value;
        retailer_agent['retailer_id'] = this.id;
        let user_retailer_json_string = JSON.stringify(retailer_agent);
