...
export class NewRetailerComponent implements OnInit {
  ...
  constructor(public mapsApiLoader: MapsAPILoader,
              ...
              private data2: RefreshDataService,
  ...
  ngOnInit() {
    ...
    console.log(this.placesAPI.getPlaces());
  ...
  getAgents() {
   ...
   this.apiService.get('users/?role_id=1')
		.subscribe(ret => {
  ...
  submitRetailerForm() {
    ...
    // this.uploadFileToServer(this.jsonImage);
    var jImage = Object.values(this.jsonImage);
    // console.log(jImage);
    // const uploadData = new FormData();
    // uploadData.append('user_id', jImage[0]);
    // uploadData.append('filename', jImage[1]);
    // uploadData.append('image_type', jImage[2]);
    // uploadData.append('image', jImage[3], jImage[1]);

    const uploadData = new FormData();
    uploadData.append('user_id', jImage[0]);
    uploadData.append('filename', jImage[1]);
    uploadData.append('image_type', jImage[2]);
    uploadData.append('image', this.convertBase64ToBlob(jImage[3], jImage[2]), jImage[1]);
    ...
    this.http.post(this.apiURL + 'images/', uploadData, options)
    .subscribe(res => {
      console.log(res);
    }, (err) => {
      console.log(err);
  ...
  uploadFileToServer(load) {
    // var files = control[0].files[0]
    this.apiService.post('images/', load)
		.subscribe(res => {
      console.log(res);
    }, (err) => {
      console.log(err);
  ...
  assignRetailerToAgent(url: string, load: string) {
    ...
    this.apiService.post(url, load)
      ...
      if(response['status'] == 201 && response['statusText'] == 'Created') {
        ...
        this.data2.changenewRetailer(true);
        this.closeModal();
      }
      else {
  ...
  convertBase64ToBlob(base64, type) {
    ...
    for(var x=0;x<byteCharacters.length;x++) {
  ...
  setImage(event) {
    ...
    if(event.target.files && event.target.files.length > 0) {
      ...
      reader.onload = () => {
        ...
        var fileData = [];
        fileData['user_id'] = 1;
        // fileData['filename'] = file.name.split('.').slice(0, -1).join('.');
        fileData['filename'] = file.name;
        fileData['image_type'] = file.type;
        // console.log(fileData['filename']);
        // fileData['image'] = reader.result.split(',')[1];
        fileData['image'] = reader.result.split(',')[1];
        // console.log(reader.result);

        let fileData_json_string = JSON.stringify(fileData);
        this.jsonImage = fileData;
      };
    }
