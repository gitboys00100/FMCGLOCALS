...
export class RoutescheddailyComponent implements OnInit {
 ...
 getDatesPerDay(day, type, daycount){
   ...
   var d1;
   var d2;
   var futuremonth = moment().add(daycount, 'M');
   var pastmonth = moment().add(daycount, 'M');
   ...
   if(type == 'next'){
       today = moment(futuremonth).startOf('month').day(day);
   }else if(type == 'prev'){
       today = moment(pastmonth).startOf('month').day(day);
   }else{
       today = moment().startOf('month').day(day);
   }

   if (today.date() > 7) today.add(7,'d');
