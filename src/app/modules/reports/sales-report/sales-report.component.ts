    ...
    .subscribe(ret => {
      ...
      if(sched == 'daily') {
        ...
        if(code == 4005) {
          ...
          var monthlyData = Object.values(data)[2];
          this.regionCounts = [];
          var currentCount = 0;
          for(var m =0;m<monthlyData.length;m++) {
            currentCount += 1;
            if(m != monthlyData.length - 1) {
              if(monthlyData[m].region != monthlyData[m+1].region) {
                this.regionCounts.push(currentCount);
                currentCount = 0;
              }
            }
            else {
  ...
  showSalesTable() {
    ...
    if (this.salesReportForm.invalid) {
      ...
      if(param_sched != 'monthly') {
        ...
        var start_of_month = format(startOfMonth(param_from_date), 'YYYY-MM-DD');
        var last_of_month = format(lastDayOfMonth(param_to_date), 'YYYY-MM-DD');
        ...
        if(param_sched == 'monthly') {
          ...
          this.selected_date_range.forEach(function(d){
            var found = false;
            monthsArray.forEach(function(m){
              if(getMonth(d) == m) {
                found = true;
              }
            });
            if(!found) {
              ...
              this.selectedMonths.push(this.getMonthFullName(getMonth(d)));
              this.selectedMonthsNumber.push(getMonth(d));
              this.headers_monthly.push(this.getMonthFullName(getMonth(d)));
            }
            monthsArray.push(getMonth(d));
            // console.log(getDate(d));
          }, this);
        }
      }
  ...
  compareDates(clicked) {
    ...
    if(from && to) {
      if(from > to) {
        ...
        alert('"From date" must be less than to "End date"!');
        if(clicked == 'from') {
    ...
    if (this.salesReportForm.invalid) {
      ...
      if(format == 'pdf') {
        ...
        if(param_sched != 'monthly') {
          ...
          this.selected_date_range = eachDay(param_from_date, param_to_date);
          // specific_date = this.removeNull(this.salesReportForm.get('specific_date').value);
          var start_of_month = this.GetFormattedDate(startOfMonth(param_from_date));
          var last_of_month = this.GetFormattedDate(lastDayOfMonth(param_to_date));
          // this.selectedMonth = this.getMonthFullName(getMonth(specific_date));
          params = new HttpParams()
            .set('agent_id', param_agent)
            .set('retailer_id', param_retailer)
        ...
        .subscribe(ret => {
          ...
          this.excelService.generateExcel('Distributor Sales Report', 'sales_report', r, header, format,'','',this.selected_date_range, param_sched, this.monthsName, this.selectedMonthsNumber);
