...
export class AnnouncementTableComponent implements OnInit {
  ...
  id: number = 0;
  e_image: string = '';
  e_company_id: number = 0;
  e_item_id: string = '';
  e_announcement_type: number = 0;
  ...
  getAnnouncements() {
    ...
    this.apiService.get('announcements/')
      ...
      this.datacount = this.announcements.length;
  ...
  onEditSubmit() {
    let retdata = {};
        ...
        retdata['start_date'] = this.profileForm.get('start').value+' 00:00:00';
        retdata['end_date'] = this.profileForm.get('end').value+' 00:00:00';
        retdata['item_id'] = this.e_item_id;
        retdata['announcement_type'] = 0;
        retdata['company_id'] = this.e_company_id;
        retdata['image'] =  this.e_image;

    let user_json_string = retdata;

    console.log(user_json_string);
    //alert(this.id);
    this.apiService.patch('announcements/'+this.id+'/', user_json_string)
    .subscribe((response) => {
...
*/
  ...
  openEditAnnouncement(item) {
    console.log(item);
    this.id = item.id;
    this.e_image = item.image;
    this.e_company_id = item.company_id;
    this.e_item_id = item.item_id;
    this.e_announcement_type = item.announcement_type;
...
//alert(item.start_date.substring(0,10));
      //this.from_date = new Date(item.start_date.substring(0,10));
      this.from_date = item.start_date.substring(0,10);
...
clickFrom(date){
  this.from_date = date;
  let control = this.profileForm.get('end');
...
checkDates(date){
  ...
  if(this.from_date <= this.to_date){
    ...
    this.profileForm.get('end').setValue('');
  }
}
