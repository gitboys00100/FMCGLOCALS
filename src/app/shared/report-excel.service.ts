...
export class ReportExcelService {
  ...
  GetFormattedDate(date: Date) {
    ...
    var month = this.getMonthFullName(todayTime.getMonth() + 1);
  ...
  generateExcel(title: string, name: string, data: any[], headers: any[], format: string, agentIndex?: string, company?: string, selected_date_range?: any[], sched?: string, monthsName?: any[], monthsNumber?: any[]) {
    ...
    if(name == 'sales_report') {
      ...
      for(var x=0;x<data.length;x++) {
        ...
        if(prevAgent != currentAgent) {
          agentRange.push(x+2);
          startAgent = (x+rowsAhead);
        }
        else {
          //merge
          // worksheet.mergeCells('A'+(x+1)':A'+(x+2));
          if((nextAgent != currentAgent) || (isLastLoopAgent)) {
            endAgent = (x+rowsAhead);
            // console.log('B'+startTownship+':B'+endTownship);
    ...
    else if(name == 'accounts_receivable') {
      if(typeof data != 'undefined') {
        data.forEach(function (d) {
          var ret = d.retailer.assigned_agents;
          var agents = '';
          ret.forEach(function(r){
            agents += r.first_name+' '+r.last_name+' ';
          });
          rowValues[1] = agents;
          // rowValues[2] = this.transformDate(d.sales_date);
          rowValues[2] = this.GetFormattedDate(d.sales_date);
          rowValues[3] = d.purchase_order_id;
          // rowValues[4] = this.transformDate(d.deadline_date);
          rowValues[4] = this.GetFormattedDate(d.deadline_date);
          rowValues[5] = d.retailer.name;
          rowValues[6] = d.amount;
          rowValues[7] = d.payment_term.name;
